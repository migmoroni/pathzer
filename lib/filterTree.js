...
function main(structure, filterType, filters) {
    ...
    function getFileSize(fileString) {
      const sizeInfo = fileString.match(/info-\(r\/w, ([0-9\.]+)\s*(bytes|KB|MB|GB)\)-info/);
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (typeof currentItem === 'string') {
          ...
          if (filterType === 'dn' && isFile) {
            const fileName = currentItem.split(';')[0].split('.')[0];
            if (wf){
              if (!filterList.includes(fileName)) {
                result.push(currentItem);
              }
            } else {
              if (!filterList.some(term => fileName.includes(term))) {
                result.push(currentItem);
              }
            }
          ...
          // Aplica os filtros para extensões de arquivos (de)
            ...
            const fileExtension = currentItem.split(';')[0].split('.').pop();
            if (wf) {
              if (!filterList.includes(fileExtension)) {
                result.push(currentItem);
              }
            } else {
              if (!filterList.some(term => fileExtension.includes(term))) {
                result.push(currentItem);
              }
            }
          ...
          // Aplica os filtros para tamanhos de arquivos (ds)
            ...
            const fileSize = getFileSize(currentItem);
            if (fileSize !== null) {
              let keepFile = true;
          ...
          // Aplica os filtros para texto de arquivos (dt)
            ...
            if (fileText !== null) {
              ...
              if (wf) {
                filterList.forEach((filter) => {
                  const regex = new RegExp(`\\b${filter}\\b`, 'i');  // Verifica palavra completa
                  if (regex.test(fileText)) {  // Comparação exata
                    keepFile = false;
                  }
                });
              } else {
