...
function getFirstNCharacters(filePath, textFile, errorReport) {
    ...
    try {
        ...
        if (captureWholeFile) {
            ...
            return fileContent == "" ? '' : `; {t(${fileContent})t}`;
        }

        // First N Chars
        if (nChars > 0 && searchWords.length === 0) {
            //return `; {t(${fileContent.slice(0, nChars)})t}`;
            return fileContent.slice(0, nChars) == "" ? '' : `; {t(${fileContent.slice(0, nChars)})t}`;
        }
        ...
        if (searchWords.length > 0) {
            ...
            return result.join('') == "" ? '' : `; {t(${result.join('')})t}`;
        }
...
function getFileInfo(fullPath, perm, size, errorReport, report) {
    ...
    return info.length > 0 ? `; {i(${info.join(', ')})}` : '';
}

// Função principal para processar o diretório e seus arquivos
function processDirectory(dirPath, progressCounter, perm = 0, size = 0, textFile = 0, errorReport = [], report) {
    ...
    directories.forEach(directory => {
        ...
        result.push(dirPathWithInfo, processDirectory(path.join(dirPath, directory), progressCounter, perm, size, textFile, errorReport, report));
    });
...
function readPath(dirPath, formatter = 0, perm = 0, size = 0, textFile = 0, report = 0) {
    const progressCounter = { count: 0 };
    const errorReport = [];

    const result = processDirectory(dirPath, progressCounter, perm, size, textFile, errorReport, report);
    console.log("\r")
