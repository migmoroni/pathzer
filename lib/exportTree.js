...
function formatDirectoryStructure(structure, formatter, ident) {
    switch (formatter) {
        case 0: return formatAsPATHZER(structure);
        ...
        case 11: return formatAsOFX(structure);
        default: throw new Error("Formato inválido!");
    }
}

// Formatter 0: FSTree
function formatAsPATHZER(array) {
...
function loadIdentConfig(ident) {
    ...
    try {
        ...
        console.error(`Read configuration error: ${saveDir}`);
        return null;
    }
}
    ...
    function processEntries(entries) {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            if (Array.isArray(entries[i + 1])) {
                ...
                const fileInfo = size ? size.replace(/\{info-\((.*?)\)-info\}/g, '$1') : '';
...
function formatAsCSV(structure) {
    ...
    csvRows.push("Path,File,Info,Content");
...
// Formatter 11: OFX
function formatAsOFX(structure) {
    ...
    function formatRecursively(arr, level = 1) {
        ...
        if (typeof currentItem === 'string') {
            ...
            const fileInfo = info.match(/\(([^)]+)\)/)[1]; // Extraindo informações do arquivo
            const fileText = text.match(/\(([\s\S]+)\)/)[1]; // Extraindo o texto do arquivo

            result += `${indent}<FILE>\n`;
            result += `${indent}  <FILENAME>${fileName}</FILENAME>\n`;
            result += `${indent}  <FILEINFO>${fileInfo}</FILEINFO>\n`;
...
function getFileNameByFormatter(formatter, nameFile) {
    switch (formatter) {
        case 0: return `${nameFile}.pathzer`;
 ...
 */
function exportTree(result, ...param){
    if (result.length != 0){
        for (let i = 0; i < (result.length); i++) {
            ...
            switch (param[0]) {
                ...
                case '/csv':
                    ...
                    break;
                case '/ofx':
                    result[i] = main(result[i], 11, ...param.slice(1));
                    break;
                case '/':
